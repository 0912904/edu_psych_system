...
WDS_URL = HF_ENDPOINT + "/datasets/clip-benchmark/wds_fer2013/resolve/main/%s/%d.tar"
WDS_SHARDS = {"test": 4, "train": 4}
...
def download_fer2013(split="test", shards=None):
    """下载 FER2013 某个划分（webdataset 分片）到 data/，返回本地 tar 路径列表。"""
    ensure_dirs()
    paths = []
    for i in range(shards or WDS_SHARDS[split]):
        dst = os.path.join(DATA, "fer2013_%s_%d.tar" % (split, i))
        if not os.path.exists(dst) or os.path.getsize(dst) < 1024:
            print("下载 FER2013 %s 分片 %d ..." % (split, i), flush=True)
            with urllib.request.urlopen(WDS_URL % (split, i), timeout=180) as r, \
                    open(dst, "wb") as f:
                f.write(r.read())
        paths.append(dst)
    return paths
...
def download_fer2013_test(shards=4):
    return download_fer2013("test", shards)


def iter_fer2013_test(cv2, limit=None):
    return iter_fer2013(cv2, "test", limit)


def iter_fer2013(cv2, split="test", limit=None):
    """逐个产出 (灰度图 ndarray, 真实类别名)。"""
    n = 0
    for tar_path in download_fer2013(split):
