...
`experiments/` 下是十一个可复现脚本，结果（CSV/JSON/PNG）在 `experiments/results/`，完整结论、表格与边界说明见 **[experiments/RESULTS.md](experiments/RESULTS.md)**。
...
| 置信度加权聚合 | `confidence_aggregation.py` | 把三条硬规则写成一个权重公式后，聚合结果稳不稳 | `confidence_aggregation.{csv,json,png}` |
| 表情+头部姿态融合 | `head_pose.py` | YuNet 关键点算出的姿态和表情是否互补 | `head_pose.{csv,json,png}` |
| 情绪转折点检测 | `transition_detection.py` | 变点检测能否替代"单步骤降"规则 | `transition_detection.{csv,json,png}` |
| 小脸鲁棒训练 | `small_face_train.py` | 多尺度退化增广能否推低可靠边界（需 PyTorch） | `small_face_train.{csv,json,png}` |
| DAiSEE 评估 | `daisee_extract.py` → `daisee_eval.py` | 系统信号与公开投入度真值的关系、能否学出可用判断 | `daisee_{zero_shot,classification,ablation}.csv`、`daisee_eval.{json,png}` |

数据集脚本会自动从 Hugging Face 拉取 FER2013 官方测试集分片（缓存在 `experiments/data/`，不入库）。消融实验需要一段本地课堂视频，仓库不提供（见下）。DAiSEE 需自行申请下载后用 `EPS_DAISEE_DIR` 指定目录；`small_face_train.py` 与 `daisee_eval.py` 需要 PyTorch（其余脚本只需 OpenCV + NumPy + matplotlib）。
